...
gt_x3 = np.array([5.0, 8.66025]) # 5 * sqrt(3)

# 측정된 거리
d12 = np.linalg.norm(gt_x1 - gt_x2)
...
init_pos = np.array([gt_x1, gt_x2, gt_x3]) + np.random.normal(0, 0.1, (3, 2))  # 초기 추정값 (약간의 노이즈 포함)

# Variable Nodes (2D)
v1 = VNode("x1", dims=[2], rho_method='covariance', init_z=init_pos[0].reshape(-1, 1), alpha_cov=10.0, rho_max=100.0)
v2 = VNode("x2", dims=[2], rho_method='covariance', init_z=init_pos[1].reshape(-1, 1), alpha_cov=10.0, rho_max=100.0)
v3 = VNode("x3", dims=[2], rho_method='covariance', init_z=init_pos[2].reshape(-1, 1), alpha_cov=10.0, rho_max=100.0)
...
f12 = DistanceFactor("f12", dims=[1], target_dist=d12, gamma=gamma_dist)
...
hist_v1, hist_v2, hist_v3 = [], [], []
total_errors = []
err1_history, err2_history, err3_history = [], [], []   
...
for i in range(n_iter):
    ...
    # Z-Consensus 값을 추출
    m1 = v1.z_consensus.flatten()
    m2 = v2.z_consensus.flatten()
    m3 = v3.z_consensus.flatten()
    ...
    # 2. 로컬 앙상블(파티클) 스캐터 플롯 
    # VNode에 연결된 첫 번째 Edge의 local_ensemble을 가져와서 퍼짐 정도 확인
    ...
    # 4. 현재 합의점 플롯
    ax.scatter(*m1, c='blue', s=150, edgecolors='black')
    ax.scatter(*m2, c='green', s=150, edgecolors='black')
    ax.scatter(*m3, c='magenta', s=150, edgecolors='black')
